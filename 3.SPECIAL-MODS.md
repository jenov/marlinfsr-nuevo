...
## 3.2 TMC2209 Single wire
  ...
  - Wiring of the StallGuard function
    
    Since the end of June 2021, this function allows the use of homing and/or probing without limit switch and without probe. To use this function, a specific firmware is required, but also a wiring between the "DIAG" pin of the TMC and the "SIGNAL" pin of the stop connector. The "Homing" function works well after setting the trigger sensitivity of the TMCs, but the "Probing" function is more difficult to implement and to set with boards which do not integrate this function (HiSpeed, NanoV1, ...).
