...
## 1.2 Validated and Actived parts
  ...
  - [x] UI Color|True| C or r | Mks-TFT32/TS35R or BTT_TFTs family
  - [x] Module Wifi|Ready|W or w|ESP3Dv2.1 or ESP3Dv3.1
  - [x] Type extruder|True|T or B|Titan or BMG extruder
  - [x] PreHeat bed before probing|True|P|60°C
  - [x] Unified Bed Leveling|True|U|32pts with memo (many location in Eeprom)
  - [x] Linear Advance|True|L| M900 K0
  - [x] Function Arc|True|R|G2 and G3 command
  - [x] Mode 32steps|UART Mode|32|Q5/QQSP/SuperRacer(All steps are doubled)
  - [x] MeatPack|Ready|M|Plugin Octoprint to compress serial dialog
  - [x] Eeprom memory|True|Config. Menu|Reset by menu
  - [x] Recovery PowerFailed|True|Config. Menu|Enable by menu 
  - [x] Classis Jerk|True|Config_Advanced Menu|10/5
  - [X] Filament sensor|True|Config. Menu|Enable by menu
  - [x] Z_OffSet Wizard|True|Special Delta Menu| Ajustment Z offset
  - [x] BabyStepping|True|By Tune Menu or Motionscreen or double touch screen|To adjust the layer height during the first print lines.
  - [x] Special Menu Delta|True|Main Menu|Easy to prepare your Delta
  - [x] Menu Host|True|Main Menu|Start a ready print on the host
  - [x] Prompt support on Host|True||Printer Action Request on Host
  - [x] All parameters|True|UI Marlin|All adjustment by menu
  - [x] Native Language|True|Main Menu|EN, FR, DE, ES, IT
  - [x] Disable Bip/Sound|True|Main Menu|A mute option to the LCD menu
  - [x] Multi-volumes|True|Media Icon or Main Menu| Enable on SuperRacer
  - [x] Thermal protection|True|Bed, Nozzle|Protect your printer from damage caused by abnormal heating.
  
  ### Option Plus Header:
  Function|State|Options|Values
  -----|---|----|---
  [ ] :star:|:ok_hand:|:tada:|:thinking:
  [x] UI Portrait|Ready|c or f| Minus letter
  [x] UI TFT|Ready|C/F or r or D| Color/Classic Marlin, Reprap Classic, Serial screen(DWIN)
  [x] Type extruder|Ready|T/B/O or X |Bowden or Direct drive system
  [x] High Temp|Ready|Hx|300°C with x = Type thermistor 
  [x] Model Prediticve Control|Ready|m| New PID nozzle
  [x] Neopixel|Ready|Nx| x = number of leds 
  [x] Auto Bed Leveling Bilinear|True|A|32 points 
  [x] Stallguard2|Ready|G|Sensorless Homing(only TMC2209) 
  [x] Stallguard2|Ready|g|Sensorless Probing(only TMC2209) 
